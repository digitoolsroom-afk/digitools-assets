/* ============================================================
   dr-freelance-page.js  —  Freelance Profile Page V3
   ============================================================ */
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function getToken() { const a = getAuth(); return a?.authToken || a?.token || a?.jwt || null; }
  ...
  function applySkeleton() {
    ["formateur-display-name","formateur-headline","average_avis","nb_project","nb_followers",
     "fp-membre-depuis","fp-last-activity","formateur-xp-years-display","fp-nb-courses","fp-nb-articles"
    ].forEach(id => {
    ...
    if (photo) photo.classList.add("fp-sk-hidden");
  }
  ...
  /* Localisation dans le hero */
  const heroLoc = $("#fp-hero-location");
  if (heroLoc) {
    const loc = [data.ville, data.pays].filter(Boolean).join(", ");
    heroLoc.textContent = loc || "—";
  }

  /* Expérience dans le hero */
  const heroXp = $("#fp-hero-xp");
  if (heroXp && data.years_experience) {
    heroXp.textContent = `${data.years_experience} ans d'expérience`;
  } else {
    const xpWrap = $("#fp-hero-xp-wrap");
  ...
  setText("#nb_project",   nbProjects ?? 0);
  setText("#nb_followers", nbFollowers ?? 0);
  ...
  if (skillsList && Array.isArray(data.sous_domaine_activity) && data.sous_domaine_activity.length > 0) {
    ...
    data.sous_domaine_activity.forEach(skill => {
      ...
      item.innerHTML = `
        ...
        <div class="fp-skill-bar"><div class="fp-skill-fill" data-pct="100"></div></div>`;
  ...
  function buildAvisItem(avis) {
    ...
    const avatarHTML = avis.image_url
      ? `<img class="fp-avis-avatar" src="${avis.image_url}" alt="" />`
      : `<div class="fp-avis-avatar-fallback">${ini}</div>`;
    const stars = Math.round(avis.note || 0);
    let starsHTML = "";
    for (let i = 0; i < 5; i++) starsHTML += `<span class="fp-avis-star ${i < stars ? "lit" : ""}">★</span>`;
    ...
    item.innerHTML = `
      ...
      <div class="fp-avis-body">
        ...
        <div class="fp-avis-stars">${starsHTML}<span class="fp-avis-note">${parseFloat(avis.note || 0).toFixed(1)}/5</span></div>
        ${comment
          ? `<div class="fp-avis-comment">${comment}</div>`
          : `<div class="fp-avis-comment" style="color:var(--text-muted);font-style:italic;">Aucun commentaire</div>`}
        <div class="fp-avis-date">${timeAgo(avis.created_at)}</div>
      </div>`;
    return item;
  }

  /* État vide avis — soigné */
  function buildAvisEmpty() {
    const div = document.createElement("div");
    div.className = "fp-avis-empty-state";
    div.innerHTML = `
      <div class="fp-avis-empty-stars">
        ${"★★★★★".split("").map(() => `<div class="fp-avis-empty-star">☆</div>`).join("")}
      </div>
      <div class="fp-avis-empty-title">Pas encore d'avis</div>
      <div class="fp-avis-empty-sub">Les avis apparaissent ici après chaque session terminée. Réservez une session pour être le premier à en laisser un.</div>
      <button class="fp-avis-empty-cta" id="fp-avis-empty-cta-btn">📅 Réserver une session</button>`;
    return div;
  }
  ...
  function renderAvis(list, containerSel, limit) {
    ...
    if (!items.length) {
      container.appendChild(buildAvisEmpty());
      /* Brancher le CTA sur le service principal */
      container.querySelector("#fp-avis-empty-cta-btn")?.addEventListener("click", () => {
        const btn = hasCoaching ? $("#coaching-book-btn") : $("#freelance-book-btn");
        if (btn) btn.click();
      });
      return;
    }
  ...
  const MAX_AVIS = 3;
  renderAvis(sortedNotations, "#avis-list", MAX_AVIS);

  const avisVoirPlus = $("#avis-voir-plus");
  if (avisVoirPlus && sortedNotations.length > MAX_AVIS) {
  ...
  function buildCourseCard(item) {
    ...
    card.href      = `https://www.digitools-room.com/pages-formation/${c.slug || ""}`;
  ...
  function buildArticleCard(item) {
    ...
    const tempsMin = Math.round((a.temps_lecture_secondes || 0) / 60) || 1;
    const nbView   = a.nb_view || 0;
    const card     = document.createElement("a");
    card.href      = `https://www.digitools-room.com/articles-de-blog/${a.slug || ""}`;
    card.target    = "_blank"; card.rel = "noopener";
    ...
    card.innerHTML = `
      ...
      <div class="fp-article-body">
        <span class="fp-content-type-badge ${isRes ? "ressource" : "article"}">${isRes ? "🔗 Ressource" : "✍️ Article"}</span>
  ...
  function buildEmptyPanel(icon, title, sub) {
    const div = document.createElement("div");
    div.className = "fp-content-empty-state";
    div.innerHTML = `
      <div class="fp-content-empty-icon">${icon}</div>
      <div class="fp-content-empty-title">${title}</div>
      <div class="fp-content-empty-sub">${sub}</div>`;
    return div;
  }

  /* Remplir les 3 panels */
  const gridCours     = $("#fp-grid-cours");
  const gridArticles  = $("#fp-grid-articles");
  const gridRessources= $("#fp-grid-ressources");

  /* Cours */
  if (gridCours) {
    if (!courses.length) {
      gridCours.parentElement.appendChild(buildEmptyPanel("📘", "Aucune formation publiée", "Les formations apparaîtront ici une fois publiées."));
      gridCours.remove();
    } else {
      courses.forEach(c => gridCours.appendChild(buildCourseCard(c)));
    }
  }

  /* Articles (sans ressources) */
  const articlesOnly = articles.filter(a => !(a.article?.is_ressource));
  if (gridArticles) {
    if (!articlesOnly.length) {
      gridArticles.parentElement.appendChild(buildEmptyPanel("✍️", "Aucun article publié", "Les articles apparaîtront ici une fois publiés."));
      gridArticles.remove();
    } else {
      articlesOnly.forEach(a => gridArticles.appendChild(buildArticleCard(a)));
    }
  }
  ...
  if (gridRessources) {
    if (!ressources.length) {
      gridRessources.parentElement.appendChild(buildEmptyPanel("🔗", "Aucune ressource publiée", "Les templates, guides et outils apparaîtront ici."));
      gridRessources.remove();
    } else {
      ressources.forEach(a => gridRessources.appendChild(buildArticleCard(a)));
    }
  }

  /* Mettre à jour les labels des tabs avec les compteurs */
  const tabEls = $$(".fp-content-tab");
  ...
  /* Tabs interactives */
  $$(".fp-content-tab").forEach(tab => {
    tab.addEventListener("click", function () {
      $$(".fp-content-tab").forEach(t => t.classList.remove("active"));
      this.classList.add("active");
      $$(".fp-content-panel-inner").forEach(p => p.classList.remove("active"));
  ...
  (function initContactModal() {
    ...
    const modalHTML = `
    <div id="contact-freelance-overlay" style="display:none;position:fixed;inset:0;z-index:9999;background:rgba(15,23,42,0.45);backdrop-filter:blur(4px);align-items:center;justify-content:center;">
      <div style="background:#fff;border-radius:20px;border:1.5px solid #bfdbfe;box-shadow:0 24px 64px rgba(37,99,235,0.15);width:100%;max-width:460px;margin:20px;font-family:'DM Sans',sans-serif;overflow:hidden;">
        ...
        <div style="padding:22px 26px;display:flex;flex-direction:column;gap:14px;">
          ...
          <button id="contact-freelance-submit" style="width:100%;padding:12px;border-radius:10px;border:none;background:#2563eb;color:#fff;font-family:'DM Sans',sans-serif;font-size:0.86rem;font-weight:600;cursor:pointer;">Envoyer et ouvrir la discussion →</button>
    ...
    triggerBtn.addEventListener("click", async () => {
      ...
      } catch { window._contactFreelanceUserId = null; }
    ...
    document.getElementById("contact-freelance-submit")?.addEventListener("click", async () => {
      ...
      if (!userId || !freelanceUserId) { errorEl.textContent = "Impossible d'identifier les participants. Réessayez."; errorEl.style.display = "block"; return; }
...
(function () {
  window.addEventListener("DOMContentLoaded", async function () {
    ...
    const id = el?.getAttribute("freelance-id");
    const idInt = parseInt(id, 10);
    if (!id || isNaN(idInt)) return;
    if (typeof sendEvent !== "function") return;
    const result = await sendEvent({ type: "freelance_page_view", freelance_id: idInt, url: window.location.href, referrer_type: document.referrer || null });
...
function initBooking(data, rules, exceptions, bookings) {
  ...
  function injectPaymentModal() {
    ...
    style.textContent = `#payment-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:10000;align-items:center;justify-content:center;padding:20px;}#payment-overlay.active{display:flex;}#payment-modal{background:#fff;border-radius:20px;padding:32px 28px 28px;max-width:460px;width:100%;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.15);font-family:'DM Sans',sans-serif;max-height:90vh;overflow-y:auto;}#payment-modal-close{position:absolute;top:16px;right:16px;background:#f4f4f0;border:none;border-radius:50%;width:32px;height:32px;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#666;}.payment-title{font-size:1.1rem;font-weight:700;color:#0f0f0f;margin-bottom:4px;}.payment-subtitle{font-size:0.82rem;color:#9a9a9a;margin-bottom:20px;}.payment-reassurance{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:12px 14px;font-size:0.78rem;color:#166534;margin-bottom:20px;display:flex;gap:8px;align-items:flex-start;line-height:1.5;}.payment-amount{background:#f8f9ff;border:1px solid #BFDBFE;border-radius:10px;padding:14px 16px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;}.payment-amount-label{font-size:0.82rem;color:#9a9a9a;}.payment-amount-value{font-size:1.1rem;font-weight:700;color:#0f0f0f;}#payment-element{margin-bottom:20px;}#payment-submit{display:block;width:100%;padding:13px;background:#2563eb;color:#fff;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:0.9rem;font-weight:600;cursor:pointer;}#payment-submit:disabled{opacity:0.6;cursor:not-allowed;}#payment-error{color:#ef4444;font-size:0.8rem;margin-top:10px;text-align:center;min-height:20px;}`;
    document.head.appendChild(style);
    const el = document.createElement('div');
    el.id = 'payment-overlay';
    el.innerHTML = `<div id="payment-modal"><button id="payment-modal-close">✕</button><div class="payment-title">💳 Paiement sécurisé</div><div class="payment-subtitle" id="payment-subtitle"></div><div class="payment-reassurance"><span>🔒</span><span>Votre carte sera autorisée mais <strong>vous ne serez débité que si le freelance accepte le rendez-vous</strong>.</span></div><div class="payment-amount"><span class="payment-amount-label">Montant à autoriser</span><span class="payment-amount-value" id="payment-amount-display"></span></div><div id="payment-element"></div><button id="payment-submit">Autoriser le paiement</button><div id="payment-error"></div></div>`;
    document.body.appendChild(el);
    document.getElementById('payment-modal-close')?.addEventListener('click', () => el.classList.remove('active'));
  ...
  function injectConfirmationModal() {
    ...
    const style = document.createElement('style');
    style.textContent = `#booking-confirm-overlay{display:none;position:fixed;inset:0;background:rgba(0,0,0,0.5);z-index:9999;align-items:center;justify-content:center;padding:20px;}#booking-confirm-overlay.active{display:flex;}#booking-confirm-modal{background:#fff;border-radius:20px;padding:32px 28px 28px;max-width:460px;width:100%;position:relative;box-shadow:0 20px 60px rgba(0,0,0,0.15);font-family:'DM Sans',sans-serif;}#booking-confirm-close{position:absolute;top:16px;right:16px;background:#f4f4f0;border:none;border-radius:50%;width:32px;height:32px;font-size:16px;cursor:pointer;display:flex;align-items:center;justify-content:center;color:#666;}.confirm-icon{font-size:2.5rem;text-align:center;margin-bottom:12px;}.confirm-title{font-size:1.2rem;font-weight:700;color:#0f0f0f;text-align:center;margin-bottom:4px;}.confirm-subtitle{font-size:0.82rem;color:#9a9a9a;text-align:center;margin-bottom:24px;}.confirm-details{background:#f8f9ff;border:1px solid #BFDBFE;border-radius:12px;padding:16px;display:flex;flex-direction:column;gap:10px;margin-bottom:20px;}.confirm-detail-row{display:flex;align-items:center;gap:10px;font-size:0.85rem;color:#0f0f0f;}.confirm-detail-icon{font-size:1rem;flex-shrink:0;width:20px;text-align:center;}.confirm-detail-label{color:#9a9a9a;font-size:0.75rem;min-width:70px;}.confirm-detail-value{font-weight:600;}.confirm-visio-note{background:#f0fdf4;border:1px solid #bbf7d0;border-radius:10px;padding:12px 14px;font-size:0.78rem;color:#166534;margin-bottom:20px;display:flex;gap:8px;align-items:flex-start;line-height:1.5;}.confirm-btn{display:block;width:100%;padding:13px;background:#2563eb;color:#fff;border:none;border-radius:10px;font-family:'DM Sans',sans-serif;font-size:0.9rem;font-weight:600;cursor:pointer;text-align:center;text-decoration:none;}`;
    document.head.appendChild(style);
    const el = document.createElement('div');
    el.id = 'booking-confirm-overlay';
    el.innerHTML = `<div id="booking-confirm-modal"><button id="booking-confirm-close">✕</button><div class="confirm-icon">🎉</div><div class="confirm-title" id="confirm-title"></div><div class="confirm-subtitle" id="confirm-subtitle"></div><div class="confirm-details" id="confirm-details"></div><div class="confirm-visio-note"><span>💡</span><span>Sur votre page <strong>Mes rendez-vous</strong> vous retrouverez toutes les informations de votre session.</span></div><a href="https://www.digitools-room.com/oauth/mes-freelances" class="confirm-btn">Voir mon rendez-vous →</a></div>`;
    document.body.appendChild(el);
    document.getElementById('booking-confirm-close')?.addEventListener('click', () => el.classList.remove('active'));
    el.addEventListener('click', e => { if (e.target === el) el.classList.remove('active'); });
  }

  function showConfirmation(bookingType, slot, duration, displayName) {
    const el = document.getElementById('booking-confirm-overlay');
    if (!el) return;
    ...
    document.getElementById('confirm-details').innerHTML = `<div class="confirm-detail-row"><span class="confirm-detail-icon">👤</span><span class="confirm-detail-label">Avec</span><span class="confirm-detail-value">${displayName}</span></div><div class="confirm-detail-row"><span class="confirm-detail-icon">📅</span><span class="confirm-detail-label">Date</span><span class="confirm-detail-value">${dateLabel}</span></div><div class="confirm-detail-row"><span class="confirm-detail-icon">🕐</span><span class="confirm-detail-label">Heure</span><span class="confirm-detail-value">${slot.displayTime}</span></div><div class="confirm-detail-row"><span class="confirm-detail-icon">⏱️</span><span class="confirm-detail-label">Durée</span><span class="confirm-detail-value">${duration} min</span></div><div class="confirm-detail-row"><span class="confirm-detail-icon">${isCoach?'🎯':'💼'}</span><span class="confirm-detail-label">Type</span><span class="confirm-detail-value">${isCoach?'Session coaching':'Rendez-vous freelance'}</span></div>`;
  ...
  function dayHasSlots(date,duration) { const dow=getLocalDow(date);const dateStr=toLocalDateStr(date);if(!rules.some(r=>r.day_of_week===dow)) return false;if(isInException(dateStr)) return false;const dur=duration||60;return rules.filter(r=>r.day_of_week===dow).some(rule=>{let cur=rule.start_time;while(cur+dur<=rule.end_time){const sMs=freelanceMinToUtcMs(dateStr,cur);const eMs=freelanceMinToUtcMs(dateStr,cur+dur);if(!isSlotBooked(sMs,eMs)) return true;cur+=dur+bufferMin;}return false;}); }
  function getSlotsForDay(date,duration) { const dow=getLocalDow(date);const dateStr=toLocalDateStr(date);const dur=duration||60;const step=dur+bufferMin;const slots=[];rules.filter(r=>r.day_of_week===dow).forEach(rule=>{let cur=rule.start_time;while(cur+dur<=rule.end_time){const sMs=freelanceMinToUtcMs(dateStr,cur);const eMs=freelanceMinToUtcMs(dateStr,cur+dur);slots.push({startUtcMs:sMs,endUtcMs:eMs,startMin:cur,endMin:cur+dur,displayTime:utcMsToVisitorTime(sMs),booked:isSlotBooked(sMs,eMs)});cur+=step;}});return slots.sort((a,b)=>a.startUtcMs-b.startUtcMs); }
  ...
  function renderStep1() {
    ...
    const minBookingMs=now.getTime()+noticeHours*3600000;const minDate=new Date(new Date(minBookingMs).getFullYear(),new Date(minBookingMs).getMonth(),new Date(minBookingMs).getDate()+1,12,0,0);
  ...
  function renderStep2() {
    ...
    step2El.innerHTML=`<div class="booking-recap-slot"><span class="booking-recap-slot-icon">📅</span><span class="booking-recap-slot-text">${dateLabel} à ${bookingState.slot?.displayTime||''}${durLabel}</span></div><div id="booking-error-banner" style="display:none;" class="booking-error-banner">⚠️ Veuillez remplir tous les champs obligatoires avant de continuer.</div><div class="booking-form" style="margin-top:20px;"><div class="form-field"><label class="form-label">Numéro de téléphone <span style="color:#ef4444;">*</span></label><div class="form-hint">Utilisé uniquement par le ${isCoach?'coach':'freelance'} en cas de besoin</div><input type="tel" class="form-input" id="booking-phone" placeholder="+33 6 12 34 56 78" value="${bookingState.phone}" /></div><div class="form-field"><label class="form-label">${noteTitle} <span style="color:#ef4444;">*</span></label><textarea class="form-textarea" id="booking-note" placeholder="${noteLabel}">${bookingState.note}</textarea></div><div class="form-field"><label class="form-label">URL de référence <span class="form-optional">(optionnel)</span></label><input type="url" class="form-input" id="booking-url" placeholder="https://votre-site.com" value="${bookingState.url}" /></div><div class="attachments-row"><div class="form-field"><label class="form-label">Pièce jointe 1 <span class="form-optional">(optionnel)</span></label><div class="file-drop-zone ${bookingState.attachment1_url?'has-file':''}" id="file-zone-1" onclick="document.getElementById('booking-file-1').click()"><div class="file-drop-zone-icon">📎</div><div class="file-drop-zone-title">Ajouter un fichier</div><div class="file-drop-zone-sub">PDF, JPG, PNG, DOCX</div><div class="file-drop-zone-success" id="file-success-1">${bookingState.attachment1_url?'✅ Fichier chargé':''}</div></div><input type="file" id="booking-file-1" style="display:none" accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx" /><input type="text" class="form-input" id="file-name-1" placeholder="Nommez votre fichier" style="margin-top:8px;display:${bookingState.attachment1_url?'block':'none'};" value="${bookingState.attachment1_name}" /></div><div class="form-field"><label class="form-label">Pièce jointe 2 <span class="form-optional">(optionnel)</span></label><div class="file-drop-zone ${bookingState.attachment2_url?'has-file':''}" id="file-zone-2" onclick="document.getElementById('booking-file-2').click()"><div class="file-drop-zone-icon">📎</div><div class="file-drop-zone-title">Ajouter un fichier</div><div class="file-drop-zone-sub">PDF, JPG, PNG, DOCX</div><div class="file-drop-zone-success" id="file-success-2">${bookingState.attachment2_url?'✅ Fichier chargé':''}</div></div><input type="file" id="booking-file-2" style="display:none" accept=".pdf,.jpg,.jpeg,.png,.webp,.doc,.docx" /><input type="text" class="form-input" id="file-name-2" placeholder="Nommez votre fichier" style="margin-top:8px;display:${bookingState.attachment2_url?'block':'none'};" value="${bookingState.attachment2_name}" /></div></div></div><div class="booking-actions"><button class="booking-btn-back" id="step2-back">← Retour</button><button class="${isCoach?'booking-btn-pay':'booking-btn-next'}" id="step2-submit">${btnLabel}</button></div>`;
  ...
  async function submitBooking() {
    ...
    if(!bookingState.phone?.trim()){if(phoneInput){phoneInput.style.borderColor='#ef4444';if(!firstErrorEl) firstErrorEl=phoneInput;}hasError=true;}else{if(phoneInput) phoneInput.style.borderColor='#BFDBFE';}
    if(!bookingState.note?.trim()){if(noteInput){noteInput.style.borderColor='#ef4444';if(!firstErrorEl) firstErrorEl=noteInput;}hasError=true;}else{if(noteInput) noteInput.style.borderColor='#BFDBFE';}
  ...
  async function createBookingItem(token) {
    const payload={freelancer_profile_id:data.id,booking_type:bookingState.type,duration_min:bookingState.duration||(bookingState.slot.endMin-bookingState.slot.startMin),price_cents:bookingState.type==='coaching'?(data.price_cents||0):0,currency:'EUR',start_at:bookingState.slot.startUtcMs,end_at:bookingState.slot.endUtcMs,client_phone:bookingState.phone||'',client_note:bookingState.note||'',reference_url:bookingState.url||'',attachment_1_url:bookingState.attachment1_url||'',attachment_1_name:bookingState.attachment1_url?bookingState.attachment1_name||'':'',attachment_2_url:bookingState.attachment2_url||'',attachment_2_name:bookingState.attachment2_url?bookingState.attachment2_name||'':'',payment_intent_id:currentPaymentIntentId||''};
    try {
      ...
      if(res.ok){document.getElementById('payment-overlay')?.classList.remove('active');overlay.classList.remove('active');showConfirmation(bookingState.type,bookingState.slot,bookingState.duration||(bookingState.slot.endMin-bookingState.slot.startMin),data.display_name);}
      else{const errData=await res.json().catch(()=>({}));alert(errData?.message||'Une erreur est survenue, veuillez réessayer.');}
