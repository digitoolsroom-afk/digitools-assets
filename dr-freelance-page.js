/* ============================================================
   dr-freelance-page.js — Profil freelance
   Layout : bannière + avatar chevauché + 2 colonnes
   Consulting affiché par défaut si les deux services sont actifs
   ============================================================ */
...
document.addEventListener("DOMContentLoaded", async function () {
  ...
  function applySkeleton() {
    SK.forEach(id => {
      ...
      sk.className = "fp-sk"; sk.style.cssText = "width:70px;height:12px;display:inline-block;border-radius:5px;";
  ...
  if (statusEl) {
    if (data.status === "active") {
      ...
      statusEl.innerHTML = `<span style="width:7px;height:7px;border-radius:50%;background:var(--orange);flex-shrink:0;display:inline-block;"></span> Hors ligne`;
    }
  }
  ...
  /* Localisation + expérience dans le hero */
  if (data.years_experience) {
    const xpWrap = $("#fp-xp-el"), xpVal = $("#fp-xp-val");
    if (xpWrap) xpWrap.style.display = "inline-flex";
    if (xpVal)  xpVal.textContent = data.years_experience + " ans d'expérience";
  }
  const loc = [data.ville, data.pays].filter(Boolean).join(", ");
  if (loc) {
    const locWrap = $("#fp-loc-el"), locVal = $("#fp-loc-val");
    if (locWrap) locWrap.style.display = "inline-flex";
    if (locVal)  locVal.textContent = loc;
  }
  ...
  const nbAvisEl = $("#nb_avis"); if (nbAvisEl) nbAvisEl.textContent = `(${nbNotation} avis)`;
  ...
  const locEl = $("#fp-meta-location"); if (locEl) locEl.textContent = loc || "—";
  if (data.linkedin_url) {
    const lw = $("#formateur-linkedin-wrap"), ll = $("#formateur-linkedin");
  ...
  const svcCard      = $("#fp-service-card");
  const paneC        = $("#tab-pane-coaching");
  const paneF        = $("#tab-pane-freelance");
  const tabsEl       = $("#fp-svc-tabs");

  if (!hasCoaching && !hasFreelance) {
    if (svcCard) svcCard.style.display = "none";
  } else if (hasCoaching && hasFreelance) {
    /* Les deux : tabs visibles, consulting par défaut */
    if (tabsEl) {
      tabsEl.style.display = "flex";
      tabsEl.innerHTML = `
        <button class="fp-svc-tab active" data-pane="tab-pane-coaching">🤝 Consulting</button>
      ...
      tabsEl.querySelectorAll(".fp-svc-tab").forEach(btn => {
        btn.addEventListener("click", function() {
          tabsEl.querySelectorAll(".fp-svc-tab").forEach(b => b.classList.remove("active"));
  ...
  /* Boutons message depuis la service card */
  ...
  function buildAvisEmpty() {
    ...
    div.innerHTML = `
      <div class="fp-avis-stars-row">
        ${[1,2,3,4,5].map(() => `<div class="fp-avis-star-box"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polygon points="12 2 15.09 8.26 22 9.27 17 14.14 18.18 21.02 12 17.77 5.82 21.02 7 14.14 2 9.27 8.91 8.26 12 2"/></svg></div>`).join("")}
      </div>
    ...
    document.getElementById("cf-submit")?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (d3 === "already exist" || d3?.message === "already exist") {
          errorEl.innerHTML = `Vous avez déjà une conversation. <a href="https://www.digitools-room.com/oauth/mes-messages" style="color:#2563eb;font-weight:700;text-decoration:underline;">Accéder à ma messagerie →</a>`;
          errorEl.style.cssText += ";background:#eff6ff;border-color:#bfdbfe;color:#2563eb;display:block;";
          submitBtn.textContent = "Envoyer →"; submitBtn.style.opacity = "1"; submitBtn.disabled = false; return;
        }
        ...
        submitBtn.textContent = "Envoyer →"; submitBtn.style.opacity = "1"; submitBtn.disabled = false;
      }
    });
  })();
...
/* ── BOOKING SYSTEM (identique à l'original) ── */
